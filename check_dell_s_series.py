...
def getSystemInfo():
    ...
    if vals:
        print(vals)
                #message.append('chassis: ' + Os10ChassisDefType.get(vals[0]) + ' (rev. ' + str(vals[1]) + ') - p/n:' + str(vals[2]) + ' - ServiceTag:' + str(vals[3]))
        message.append('chassis: ' + ' (rev. ' + str(vals[1]) + ') - p/n:' + str(vals[2]) + ' - ServiceTag:' + str(vals[3]))
    else:
        retCode = 3
        message.insert(0, 'Unable to get SNMP metrics from server !')
    ...
    if vals:
        ...
        cardDescr = "S5148F-ON 48x25GbE, 6x100GbE QSFP28 Interface Module" # DEBUG
        cardStatus = int(1)
        #message.append('card: ' + vals[0] + ' (rev. ' + vals[1] + ') - p/n:' + vals[2] + ' - ServiceTag:' + vals[4] + ' - status:' + Os10CardOperStatus.get(vals[3]))
        message.append('card: ' + str(cardDescr) + ' (rev. ' + str(vals[1]) + ') - p/n:' + str(vals[2]) + ' - ServiceTag:' + str(vals[4]) + ' - status:' + Os10CardOperStatus.get(vals[3]))
    else:
        retCode = 3
        message.insert(0, 'Unable to get SNMP metrics from server !')
...
def getTemperatures(warn, crit):
    ...
    #vals = snmpSession.walk(vars) # doesn't work with walk mode
    vals = snmpSession.get(vars)
    if vals:
        for temp in vals:
            #print("DEBUG getTemperatures:" + temp)
